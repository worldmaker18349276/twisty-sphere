...
// builder.selectElement(puzzle, display, obj=>console.log(obj));

// var s = new THREE.BoxGeometry(20,20,20);
// cutConvexPolyhedron(s, builder.plane(1,1,1,1), true);
// display.scene.add(new THREE.Mesh(s, new THREE.MeshLambertMaterial()));

function loop() {
  setTimeout(() => {
    builder.analyze(puzzle);
    builder.selectPlane(puzzle, display,
      (_, x) => puzzle.userData.angles[x],
      (_, x) => builder.twist(puzzle, x, 0, display, loop)
    );
  }, 100);
}
loop();
