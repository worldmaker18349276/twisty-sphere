...
class Geometer
{
  ...
  // 
  // static face_intersect(geometry1, face1, geometry2, face2) {
  //   var vec1 = VERTICES.map(a => geometry1.vertices[face1[a]]);
  //   var vec2 = VERTICES.map(a => geometry2.vertices[face2[a]]);
  //   var tri1 = new THREE.Triangle().setFromPointsAndIndices(geometry1.vertices, face1.a, face1.b, face1.c);
  //   var tri2 = new THREE.Triangle().setFromPointsAndIndices(geometry2.vertices, face2.a, face2.b, face2.c);
  //   var plane1 = tri1.getPlane();
  //   var plane2 = tri1.getPlane();
  // 
  //   var dis1 = vec1.map(v => plane2.distanceToPoint(v));
  //   var dis2 = vec2.map(v => plane1.distanceToPoint(v));
  //   var sgn1 = dis1.map(d => Math.abs(d)<tol ? 0 : Math.sign(d));
  //   var sgn2 = dis2.map(d => Math.abs(d)<tol ? 0 : Math.sign(d));
  // 
  //   if ( sgn1.every(s => s == 0) || sgn2.every(s => s == 0) ) {
  //     ...
  //   }
  // 
  //   if ( sgn1.every(s => s >= 0) || sgn1.every(s => s <= 0) )
  //     return;
  //   if ( sgn2.every(s => s >= 0) || sgn2.every(s => s <= 0) )
  //     return;
  // 
  //   var inter1 = EDGES.map(([a, b]) => [VERTICES_IND[a], VERTICES_IND[b]])
  //                     .filter(([i, j]) => sgn1[i] == 0 || sgn1[i] * sgn1[j] < 0)
  //                     .map(([i, j]) => sgn1[i] == 0 ? [i, j, 0] : [i, j, Math.abs(dis1[j]/(dis1[i]-dis1[j]))]);
  //   var inter2 = EDGES.map(([a, b]) => [VERTICES_IND[a], VERTICES_IND[b]])
  //                     .filter(([i, j]) => sgn2[i] == 0 || sgn2[i] * sgn2[j] < 0)
  //                     .map(([i, j]) => sgn2[i] == 0 ? [i, j, 0] : [i, j, Math.abs(dis2[j]/(dis2[i]-dis2[j]))]);
  //   var points1 = inter1.map(([i,j,t]) => t == 0 ? vec1[i] : new THREE.Vector3().lerpVectors(vec1[i], vec1[j], t));
  //   var points2 = inter2.map(([i,j,t]) => t == 0 ? vec2[i] : new THREE.Vector3().lerpVectors(vec2[i], vec2[j], t));
  // 
  //   if ( points1.some(p => tri2.containsPoint(p)) || points2.some(p => tri1.containsPoint(p)) ) {
  //     ...
  //   }
  // }
  // 
