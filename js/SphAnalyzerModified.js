 ...
 * element without boundary indicate full space of spherical surface.  Notice that
 * standard representation shouldn't have trivial vertices, self-adjacent edges,
 * widthless bridge, sandglass bridge, and unconnected components, but they are
 * still fixable.  Some algorithms are only valid for standard representation.
 *
 * @class
...
class SphSeg
{
  ...
  split(...groups) {
    var elements = groups.map(group => new Set(group))
                         .filter(group => group.size);
    var aff = this.affiliation;
  ...
  *fly() {
    var segs = new Set(this.affiliation);
 ...
 * It represent the connected region defined by boundaries (BREP).  The element
 * without boundary indicate full space of spherical surface.
 * Notice that self-adjacent edges, widthless bridge and sandglass bridge are
 * invalid.
 *
 * @class
...
/**
 * Rail of spherical twisty puzzle.
 ...
 *
 * @class
 * @property {Set<SphSeg>} segments - Segments of rail.
 * @property {Set<SphSeg>} shield - The unchanged part.
 * @property {Set<SphSeg>} weaknesses
 * @property {Map<SphSeg,SphSeg[]>} crack - Segments at a vertex intersecting this rail.
 * @property {SphCircle} circle - Circle of rail.
 */
class SphRail
{
  constructor() {
...
class SphAnalyzer
{
  ...
  *loops(segs) {
  ...
  jump(seg0, theta, prefer=+1) {
    if ( theta == 0 && prefer > 0 ) {
      let [[adj_seg, offset]] = seg0.adj;
      return [adj_seg, offset];

    } else if ( theta == seg0.arc && prefer < 0 ) {
      let [adj_seg, offset];
      for ( [adj_seg, offset] of seg0.adj );
      console.assert(offset >= seg0.arc);
      offset = 0;
      for ( let [seg_, offset_] of adj_seg.adj ) {
        if ( seg_ === seg0 ) break;
        offset = offset_;
      }
      return [adj_seg, offset];
      ...
      for ( let [adj_seg, offset] of seg0.adj ) {
        let theta_ = this.mod4(offset-theta, [0, adj_seg.arc, ...adj_seg.adj.values()]);
   ...
   *   point.
   * @param {number} [dir=+1] - Direction of spin: `+1` means clockwise.
  ...
  *spin(seg, offset=0, prefer=+1, dir=+1) {
  ...
  relationTo(circle1, circle2) {
    ...
    if ( this.cmp(distance, 0) == 0 && this.cmp(radius1, radius2) == 0 )
      ...
      return [abcA(distance, radius1, radius2),
              abcA(radius2, radius1, distance)*2,
              abcA(radius1, radius2, distance)*2, 2]; // intersect
    else
   ...
   * @param {number} side - The side before meeting, is one of [+1, +0, -0, -1].
   *   It will not classify this meet if this parameter is absent.
  ...
  classifyMeet(meet, side) {
    ...
    var side_ = (meet.type===undefined ? 0 : 1) * Math.sign(1/meet.angle);
    if ( side === undefined )
      return side_;

    var pre, dir, post;
    post = Object.is(side_, -0) ? -2 : side_;
    pre  = Object.is(side , +0) ? +2 : side ;

    if ( Math.sign(1/side) != Math.sign(1/side_) )
      dir = 0;
    else if ( Math.sign(1/side) > 0 )
  ...
  *meetWith(segment0, circle) {
    ...
    const snapPos = (meet, offset_, theta_) => {
      if ( fzy_cmp(abs4(meet.offset-offset_), 0, this.tol*10) )
        console.warn(`calculation error is too large: ${abs4(meet.offset-offset_)}`);
      meet.offset = offset_;
      if ( fzy_cmp(abs4(meet.theta-theta_), 0, this.tol*10) )
        console.warn(`calculation error is too large: ${abs4(meet.theta-theta_)}`);
      meet.theta = theta_;
    };
    ...
    for ( let segment of segment0.walk() ) {
      // find meets of vertices
      let ticks = Array.from(segment.adj).filter(([,offset]) => offset < segment.arc);
      ticks.unshift([segment, 0]);
      if ( segment.arc != 4 )
        ticks.push([segment.next, segment.arc]);
      ticks = ticks.map(([seg, offset]) =>
        [this.cmp(circle.radius, angleTo(circle.center, seg.vertex)/Q),
         offset, this.mod4(circle.thetaOf(seg.vertex))]);

      // find meets of segment
      let circle_ = segment.circle;
      let [angle, arc, arc_, type] = this.relationTo(circle_, circle);
      let offset, theta;

      if ( type === undefined ) {
        console.assert(ticks.every(m => m[0] == 0));
        ...
        console.assert(Math.abs(ticks.reduce((a,m)=>a+m[0], 0)) == ticks.length);
        // nothing

      } else if ( type == 1 ) {
        theta = this.mod4(circle.thetaOf(circle_.center)+arc_/2);
        offset = this.mod4(circle_.thetaOf(circle.center)-arc/2);
        console.assert(Math.abs(ticks.reduce((a,m)=>a+m[0], 0)) >= ticks.length-1);
        ...
        let meet = {angle, segment, offset, theta, type};
        for ( let [s, offset_, theta_] of ticks ) if ( s == 0 )
          snapPos(meet, offset_, theta_);
        ...
        for ( let [s, offset_, theta_] of ticks ) if ( s == 0 )
          snapPos(abs4(meet1.offset-offset_)<abs4(meet2.offset-offset_) ? meet1 : meet2, offset_, theta_);
  ...
  interpolate(seg, theta) {
    ...
    for ( let [adj_seg, offset] of seg.adj ) {
      ...
      // add adjacent of splitted
      let offset_ = this.mod4(offset - seg.arc, [4, splitted.arc, adj_seg.arc]);
      if ( this.cmp(offset_, splitted.arc + adj_seg.arc) < 0 )
        splitted.adjacent(adj_seg, offset_);
    }
  ...
  // mergeTrivialVertices(segs) {
  //   for ( let seg of segs ) if ( seg.prev !== seg && this.isTrivialVertex(seg) )
  //     this.mergePrev(seg);
  // }
  ...
  /**
   * Find contacts between two adjacent segments.
   *
  ...
  contactsBetween(seg1, seg2) {
    ...
    for ( let i=0; i<keys.length; i+=2 ) {
      ...
      contacts.push([seg1, theta1, theta1_, seg2, theta2_, theta2]);
    }

    return contacts;
  }
  /**
   * Find the pairs of segments that are connected by adjacent relation.
   *
   * @param {SphSeg} seg0 - The segment to zip.
   * @param {SphSeg[]} segs - The segments to zip.
   * @returns {SphSeg[][]} The pairs of segments.
   */
  findZippers(seg0, segs=seg0.affiliation) {
    segs = new Set(segs);
    var pairs = [];
    ...
    // return pairs.flatMap(([seg1, seg2]) => this.contactsBetween(seg1, seg2));
    return pairs;
  }
  /**
   * Glue adjacent segments.
   *
   * @param {object[]} contacts - The data returned by {@link SphAnalyzer#contactsBetween}.
  ...
  glueAdj(contacts) {
    ...
    var pairs = [];
    for ( let [seg1, theta1, theta1_, seg2, theta2, theta2_] of contacts ) {
    ...
    for ( let [seg1, seg2] of pairs ) {
      ...
      seg1.split([seg1, seg2]);
      if ( seg1.forward )
        seg1.forward.backward = undefined;
      if ( seg1.backward )
        seg1.backward.forward = undefined;
      if ( seg2.forward )
        seg2.forward.backward = undefined;
      if ( seg2.backward )
        seg2.backward.forward = undefined;
  ...
  // fuseElems(...segs) {
  //   var elems = new Set(segs.map(seg0 => seg0.affiliation));
  //   var contacts = [];
  //   for ( let elem of elems ) for ( let seg of elem )
  //     for ( let adj_seg of seg.adj.keys() )
  //       if ( elems.some(elem => elem.has(adj_seg)) )
  //         contacts.push(...this.contactsBetween(seg, adj_seg));
  // 
  //   this.glueAdj(contacts);
  // }
  ...
  sketchProfile(segments) {
    ...
    var uncovered = [];
    for ( let seg of segments )
      for ( let [seg_, offset] of this.adj )
        if ( !segments.has(seg_) )
          uncovered.push(...this.contactsBetween(seg, seg_));
   ...
   * @returns {boolean} True if point is inside this region, or undefined if it
   *   is at the boundary of this region.
   */
  ...
  splitByConectivity(boundaries) {

  }

  /**
   * Draw maximal dashes of elements by given sorted meets.
   * Dash is array of meets that indicate where to slice.  The order of meets in
   * dash should be same as given sorted meets.
   ...
   *
   * @param {object[]} meets - The sorted meets.
   * @returns {object[][]} Dashes, which are arrays of meets.
   */
  dashesOf(meets) {
    var dashes = [];
    if ( meets.some(meet => meet.type[1] == "|") ) {
      while ( !["(|)", "(|]", "[|)", "[|]"].includes(meets[0].type) ) {
        let meet = meets.shift();
      ...
      for ( let meet of meets ) {
        if ( ["(|)", "(|]", "[|)", "[|]"].includes(meet.type) )
          dash = [meet];
        else if ( [")|(", "]|(", ")|[", "]|["].includes(meet.type) )
          dash.push(meet), dashes.push(dash);
        else if ( ["(<(", ")<)"].includes(meet.type) )
          dash.push(meet);
        else
          console.assert(dash === undefined);
      }
      console.assert(dash === undefined);

    } else if ( !meets.some(({type}) => type[1] == ">")
                || meets.some(({type}) => type[1] == "<")
                || this.contains(elem.boundaries, circle.vectorAt(0)) ) {
      // draw dashes
      let dash = [];
      for ( let meet of meets ) {
        if ( ["(<(", ")<)"].includes(meet.type) )
  ...
  sliceAlong(seg0, dash, circle) {
    var circle_ = new SphCircle(circle).complement();

    // interpolation
    for ( let meet of dash ) if ( meet.offset != 0 )
      [meet.segment, meet.offset] = [this.interpolate(meet.segment, meet.offset), 0];

    // 0-length dash
    if ( dash[0] !== dash[dash.length-1] && dash[0].theta == dash[dash.length-1].theta ) {
      let start = dash[0];
      let end = dash[dash.length-1];
      let inner = dash.slice(1, -1).filter(meet => meet.type == "(<(");
      let outer = dash.slice(1, -1).filter(meet => meet.type == ")<)");

      this.swap(start.segment, end.segment,
                end.angle-start.angle-4, start.angle-end.angle);
      for ( let meet of inner )
        this.swap(meet.segment, end.segment,
                  end.angle-meet.angle, meet.angle-end.angle-4);
      for ( let meet of outer.reverse() )
        this.swap(meet.segment, start.segment,
                  start.angle-meet.angle, meet.angle-start.angle-4);

      return;
    }

    // draw lines
    let inner, outer, in_seg, out_seg, in_theta, out_theta;
    if ( dash.length == 0 || dash[0] === dash[dash.length-1] ) {
      // full-circle dash
      inner = dash.slice(0, -1).filter(meet => meet.type == "(<(");
      outer = dash.slice(0, -1).filter(meet => meet.type == ")<)");
      in_theta = inner[0] ? inner[0].theta : 0;
      out_theta = outer[outer.length-1] ? outer[outer.length-1].theta : 0;

      // make segments between two meets
      in_seg  = new SphSeg({radius:circle.radius,  arc:4, angle:2});
      out_seg = new SphSeg({radius:circle_.radius, arc:4, angle:2});
      in_seg.orientation = q_spin(circle.orientation, in_theta*Q);
      out_seg.orientation = q_spin(circle_.orientation, -out_theta*Q);
      seg0.merge(in_seg, out_seg);
      in_seg.connect(in_seg);
      out_seg.connect(out_seg);
      in_seg.align(in_seg);
      out_seg.align(out_seg);
      in_seg.adjacent(out_seg, this.mod4(out_theta-in_theta, [4]));

    } else {
      // segment dash
      let start = dash[0];
      let end = dash[dash.length-1];
      inner = dash.slice(1, -1).filter(meet => meet.type == "(<(");
      outer = dash.slice(1, -1).filter(meet => meet.type == ")<)");
      in_theta = start.theta;
      out_theta = end.theta;
      let arc = this.snap(out_theta-in_theta, [4]);

      // make segments between two meets
      in_seg  = new SphSeg({radius:circle.radius,  arc, angle:4});
      out_seg = new SphSeg({radius:circle_.radius, arc, angle:4});
      in_seg.orientation = q_spin(circle.orientation, in_theta*Q);
      out_seg.orientation = q_spin(circle_.orientation, -out_theta*Q);
      seg0.merge(in_seg, out_seg);
      ...
      this.swap(in_seg, start.segment, start.angle, -start.angle-4);
      this.swap(end.segment, out_seg, -2-end.angle, -2+end.angle);
    }

    // draw dashes
    for ( let meet of inner.reverse() ) {
      let offset = this.snap(meet.theta-in_theta, [0, in_seg.arc]);
      if ( offset == in_seg.arc )
        this.swap(in_seg.next, meet.segment,
                  meet.angle-(2-in_seg.next.angle)-4, (2-in_seg.next.angle)-meet.angle);
      else if ( offset == 0 )
        this.swap(in_seg, meet.segment,
                  meet.angle-4, -meet.angle);
      else
        this.swap(this.interpolate(in_seg, offset), meet.segment,
                  meet.angle-4, -meet.angle);
    }
    for ( let meet of outer ) {
      let offset = this.snap(out_theta-meet.theta, [0, out_seg.arc]);
      if ( offset == out_seg.arc )
        this.swap(out_seg.next, meet.segment,
                  out_seg.next.angle+meet.angle-4, -out_seg.next.angle-meet.angle);
      else if ( offset == 0 )
        this.swap(out_seg, meet.segment,
                  (2+meet.angle)-4, -(2+meet.angle));
      else
        this.swap(this.interpolate(out_seg, offset), meet.segment,
                  (2+meet.angle)-4, -(2+meet.angle));
    }
  }
  // sliceElem(seg0, circle, is_proper=false) {
  //   var paths = Array.from(seg0.fly())
  //                    .map(([seg0]) => Array.from(this.meetWith(seg0, circle)));
  //   var meets = this.sortMeets(paths, circle);
  // 
  //   var dashes = this.dashesOf(meets);
  ...
  twistAdjacenciesAlong(seg0, angle) {
    var inner = full(seg0.ski());
    var [[seg0_, shift]] = seg0.adj;
    var outer = full(seg0_.ski());
    if ( !inner || !outer )
  ...
  rotationsOfTwist(op, hold) {
    ...
    var tracks = Array.from(op.keys()).map(seg0 => [seg0, seg0.adj.keys().next().value])
      .map(([seg1, seg2]) => [full(seg1.ski()), full(seg2.ski())]);
    console.assert(tracks.every(([inner, outer]) => inner && outer));
    var partition = this.partitionByBoundaries(...tracks.flat());
  ...
  */
  rotationsOfTwist(op, hold) {
    ...
    var partition = this.partitionByBoundaries(...tracks.flatMap(track => [track.inner, track.outer]));
    var region0 = partition.find(region => region.elements.has(hold)) || partition[0];
    region0.rotation = [0,0,0,1];
    var rotated = new Set([region0]);
  ...
  isTwistable(segment, boundaries=segment.affiliation.boundaries, global_test=false) {
    ...
    if ( global_test ) {
      var meets = Array.from(this.loops(boundaries))
                       .map(loop => Array.from(this.meetWith(loop[0], segment.circle)));
  ...
  buildTrack(seg) {
    ...
    if ( this.cmp(seg.radius, 1) > 0 )
      [seg, seg_] = [seg_, seg];
    var inner = full(this.ski(seg));
    if ( !inner ) return;
    var outer = full(this.ski(seg_));
    if ( !outer ) return;
...
class SphPuzzle
{
  ...
  clean(level=2) {
    ...
    if ( level >= 1 ) {
      ...
      for ( let group of this.analyzer.partitionByTwistability(this.brep.elements, level>=2) )
        if ( group.length > 1 )
  ...
  decipher(track, level=1) {
    ...
    var partition = this.analyzer.partitionByBoundaries(track.inner, track.outer);
