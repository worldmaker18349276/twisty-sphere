...
Object.assign(Display.prototype, {
  add: function(target, parent=this.scene) {
    parent.add(target);
    target.dispatchEvent({name:"objectadded", target, parent});
  },
  remove: function(target, parent=this.scene) {
  ...
  lockTrackball: function(key) {
    this.trackball_lock.add(key);
  },
  unlockTrackball: function(key) {
    this.trackball_lock.delete(key);
  },

  animate: function(coroutine) {
    return new Promise((resolve, reject) => {
      this.animations.push([coroutine, resolve, reject]);
    });
  },
  animatedRotateRoutine: function*(targets, axis, angle, speed) {
