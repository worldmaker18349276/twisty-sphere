...
class It
{
  ...
  static from(iter) {
    return new It(iter[Symbol.iterator]);
  }
  static keys(col) {
    if ( col instanceof Map || col instanceof Set || Array.isArray(col) )
      return new It(col.keys());
    else
      return new It(Object.keys(col)[Symbol.iterator]);
  }
  static values(col) {
    if ( col instanceof Map || col instanceof Set || Array.isArray(col) )
      return new It(col.values());
    else
      return new It(Object.values(col)[Symbol.iterator]);
  }
  static entries(col) {
    if ( col instanceof Map || col instanceof Set || Array.isArray(col) )
      return new It(col.entries());
    else
      return new It(Object.entries(col)[Symbol.iterator]);
  }

  static count(start=0, end=Infinity, step=0) {
    return new It(function*() { for ( let i=start; i<end; i+=step ) yield i; });
  }
  static cycle(...items) {
    return new It(function*() { while ( true ) yield* items; });
  }
  static chain(...iters) {
    iters = iters.map(it => it[Symbol.iterator]);
    return new It(function*() { for ( let iter of iters ) yield* iter; });
  }
  static zip(...iters) {
    iters = iters.map(it => it[Symbol.iterator]);
    return new It(function*() {
      let res = iters.map(iter => iter.next());
      while ( res.every(r => !r.done) ) {
        yield res.map(r => r.value);
        res = iters.map(iter => iter.next());
      }
    });
  }

  map(func) {
    let iterator = this.origin;
    return new It(function*() { let i=0; for ( let value of iterator ) yield func(value, i++); });
  }
  filter(func) {
    let iterator = this.origin;
    return new It(function*() { let i=0; for ( let value of iterator ) if ( func(value, i++) ) yield value; });
  }
  reduce(func, init) {
    let i = 0;
    if ( init === undefined )
      init = this.origin.next().value, i++;
    for ( let item in this.origin )
  ...
  findLast(func, otherwise) {
    return this.reduce((res, item, i) => func(item, i) ? item : res, otherwise);
  }
  findLastIndex(func) {
  ...
  // }
  // max(cmp) {
  //   return this.reduce((a, b) => cmp(a, b) >= 0 ? b : a);
  // }
  // groupsort(cmp) {}
  // chuck(keyfunc) {}
  // 
  // length() {
  //   return this.reduce((,,i) => i+1);
  // }
  // isEmpty() {
  ...
  toUniqArray() {
    return Array.from(new Set(this.origin));
  }
  toSet() {
    return new Set(this.origin);
  }
  toMap() {
    return new Map(this.origin);
  }
  toObject() {
    let object = {};
    for ( let [key, value] of this.origin ) {
      object[key] = value;
    }
    return object;
  }
...
// vector
function disL1([x1, y1, z1], [x2, y2, z2]=[0,0,0]) {
 ...
 * property `siblings`.
 * A point on the segment always be decribed by a list `[segment, offset, prefer]`,
 * called vertex.  The coordinate of this point is `segment.vectorAt(offset)`,
 * and `prefer` the sign represents the orientation of the point, which can be
 * thought of as the perturbation:
 * `rotate(segment.vectorAt(offset), quaternion(segment.center, prefer * tol))`.
 *
 ...
 * @property {number[]} vertex - The starting vertex of this segment.
 * @property {SphCircle} circle - The extended circle of this segment.
 ...
 *   surrounding by those segments describes an element of puzzle, so this set
 *   also represent the host element of this segment.
 */
class SphSeg extends SphCircle
{
  ...
  split(...groups) {
    ...
    let aff = this.siblings;
    for ( let element of elements )
      for ( let segment of element )
        aff.delete(segment), segment.siblings = element;
    return elements;
  }
  connect(seg) {
...
class SphAnalyzer
{
  ...
  *loops(segs) {
  ...
  jump(seg0, theta, prefer=+1) {
    if ( theta == 0 && prefer > 0 ) {
      let [offset, adj_seg] = It.entries(seg0.adj).first();
      return [adj_seg, offset];

    } else if ( theta == seg0.arc && prefer < 0 ) {
      let [offset, adj_seg] = It.entries(seg0.adj).last();
      offset = It.keys(adj_seg.adj).findLast(offset_ => offset_ < offset, 0);
      return [adj_seg, offset];
      ...
      let [offset, adj_seg] = It.keys(seg0.adj).find(offset => offset > theta);
  ...
  *spin(seg, offset=0, prefer=+1, dir=+1) {
   ...
   *
   * @param {SphCircle} knife - The circle to compare.
   * @param {number[]} point - The point to compare.
   * @returns {number} The value representing their relation: `REL.INSIDE`
  ...
  meetWithPoint(knife, point) {
    let sgn = this.cmp(knife.radius, angleTo(knife.center, point)/Q);
    if ( sgn > 0 )
      return REL.INSIDE;
    else if ( sgn < 0 )
      return REL.OUTSIDE;
   ...
   *
   * @param {SphCircle} knife - The first circle to compare.
   * @param {SphCircle} circle - The second circle to compare.
   * @returns {Array} A tuple of value and points.
  ...
  meetWithCircle(knife, circle) {
    ...
    if ( this.cmp(distance, 0) == 0 && this.cmp(radius1, radius2) == 0 )
      [rel, offset, num] = [REL.INLINE,  null, null]; // equal
    else if ( this.cmp(distance, 2) == 0 && this.cmp(radius1 + radius2, 2) == 0 )
      [rel, offset, num] = [REL.OUTLINE, null, null]; // complement
    else if ( this.cmp(radius1, this.abs4(distance + radius2)) >  0 )
   ...
   *
   * @param {SphCircle} knife - The circle to compare.
   * @param {Array<Set<SphSeg>>} elements - The elements to compare.
   * @returns {Array} The zippers of meeted segments.
   */
  meetWithElements(knife, elements) {
    elements = Array.from(elements);
    let boundaries = elements.flatMap(elem => Array.from(elem));
    let vertices = [];
    let efink = new SphCircle(knife).flip();


    // meet with points
    /**
     * The relation between vertices and knife.
     * The vertex is denoted by list `[segment, offset, prefer]` (see document
     * of {@link SphSeg}).
     * The relation is almost same as the return value of function
     * {@link SphAnalyzer#meetWithPoint}:
     ...
     * more closed to inside/outside (determined by `prefer` and `segment`);
     * `REL.BOTHSIDE` means the vertex is on the edge of knife, but unable to
     * classify the side now; it should be dealt with later.
     *
     * @var {Map<Array,number>} vrels - The map from vertex to relation.
     */
    let vrels = new Map();
    {
      ...
      for ( let segment of boundaries ) {
        ...
        for ( let offset of offsets )
          vs.add([segment, offset, +1]);
      }

      // group vertices by their position
      for ( v0 of vs ) {
        let vgroup = [];
        for ( let [, ...v] of this.spin(...v0) )
          if ( boundaries.includes(v[0]) ) {
            let v_ = It.from(vs).find(v_ => Array.equal(v_, v));
            console.assert(v_ !== undefined);
            vs.delete(v_);
            vgroup.push(v);
          }
        // only collect the vertices on the edge of segments and the self-intersected points.
        if ( v0[1] === 0 || vgroup.length > 2 )
      ...
      for ( let vgroup of vertices ) {
        let v0 = vgroup[0];
        let rel = this.meetWithPoint(knife, v0[0].vectorAt(v0[1]));
        for ( let v of vgroup ) {
          console.assert(rel === this.meetWithPoint(knife, v[0].vectorAt(v[1])));
          vrels.set(v, rel);
        }
      }
    }
    ...
    // meet with circles
    /**
     * The relation between extended circle of segment and knife.
     * The relation is same as return of function
     * {@link SphAnalyzer#meetWithCircle}:
     *
     * @var {Map<SphSeg,number>} crels - The map from segment to relation.
     */
    let crels = new Map();
    for ( let segment of boundaries ) {
      let children = It.keys(vrels).filter(k => k[0] === segment).toArray();
      let [rel, meets] = this.meetWithCircle(knife, segment);
      crels.set(segment, rel);

      if ( rel === REL.INLINE || rel === REL.OUTLINE ) {
        // overlap: determine the side of vertices
        console.assert(children.every(v => vrels.get(v) === REL.BOTHSIDE));
        for ( let v of children ) vrels.set(v, rel);

      } else if ( meets.length === 0 ) {
        // no meet
        console.assert(children.every(v => vrels.get(v) === rel));

      } else {
        // intersected at some points

        // snap meet points to known vertices on the knife
        let snapped = {[+1]:[], [-1]:[]};
        for ( let v of children ) if ( vrels.get(v) === REL.BOTHSIDE ) {
          let p = v[0].vectorAt(v[1]);
          let snapped_ = snapped[v[2]];
          let [meet, dis] = meets.map(m => [m, disL1(m, p)])
                                 .sort((a, b) => a[1]-b[1]).shift();
          console.assert(!snapped_.includes(meet));
          snapped_.push(meet);
          if ( fzy_cmp(dis, 0, this.tol*10) )
            console.warn(`calculation error is too large: ${dis}`);

          // determine the side of vertices
          let s = meets[0] === meet ? v[2] : -v[2];
          if ( rel === REL.BOTHSIDE )
            vrels.set(v, s > 0 ? REL.INLINE : REL.OUTLINE);
          else
            vrels.set(v, rel);
        }
        meets = meets.filter(m => !snapped[+1].includes(m) && !snapped[-1].includes(m));

        // make vertices for remaining meet points
        for ( let meet of meets ) {
          let offset = this.mod4(segment.thetaOf(meet));
          if ( offset >= segment.arc )
            continue;

          // the vertices around this meet point
          let vgroup = It.from(this.spin(segment, offset, +1)).map(a => a.slice(1))
                         .filter(v => boundaries.includes(v[0])).toArray();
          vertices.push(vgroup);

          for ( let v of vgroup ) {
            console.assert(v[0] === segment || !crels.has(v[0]));
            console.assert(It.keys(vrels).every(k => !Array.equal(k, v)));
            console.assert(REL.BOTHSIDE === this.meetWithPoint(knife, v[0].vectorAt(v[1])));

            // determine the side of vertices
            let s = meets[0] === meet ? v[2] : -v[2];
            if ( v[0] !== segment || v[1] !== offset )
    ...
    /**
     * This map simulate the first yield value of spin of meeted vertex after
     * slicing.
     * The key of map has the form `[angle, circle, theta, prefer]`, which has
     * the same form of yield value of {@link SphAnalyzer#spin}, where `circle`
     * can be `knife` or `efink`.
     * The value can be meeted vertex we previous computed or element.
     * Starting from direction of meeted vertex, one can spin around the meeted
     * point clockwisely/counter-clockwisely (depends on prefer of vertex), and
     * should reach the knife with yield value as the corresponding key in th map.
     * If the value of map is element, it means there is no vertex can reach it.
     *
     * @var {Map<Array,Object>} angles - The map from spin to vertex or element.
     */
    let spins = new Map();
    let thetas = [];
    {
      ...
      for ( let vgroup of vmeets ) {
        // signed angle from knife to vertex, the sign of angle is equal to the side of vertex
        let angs = [];
        {
          // tangent vector along knife
          let v0 = vgroup[0];
          let theta = this.mod4(knife.thetaOf(v0[0].vectorAt(v0[1])));
          thetas.push(theta);
          let dv0 = rotate([Math.cos((theta+1)*Q), Math.sin((theta+1)*Q), 0], knife.orientation);

          for ( let v of vgroup ) {
            // tangent vector along segment
            let [seg, th, p] = v;
            let dv = rotate([Math.cos((th+p)*Q), Math.sin((th+p)*Q), 0], seg.orientation);

            let sgn = vrels.get(v) === REL.INLINE ? +1 : -1;
            let ang = angleTo(dv, dv0)/Q;
            ang = ang * sgn;
            if ( crels.get(seg) !== REL.BOTHSIDE )
              ang = this.abs4(ang) < 1 ? 0 : 2;
            else
              ang = this.mod4(ang + 1) - 1;
            ang = ang * sgn;
        ...
        for ( let i=0; i<vgroup.length; i+=2 ) {
          ...
          let [seg1, th1, p1] = v1;
          let [seg2, th2, p2] = v2;
          console.assert(p1===-1 && p2===+1);
          console.assert(th1===seg1.arc ? (seg2===seg1.next && th2===0) : (seg2===seg1 && th2===th1));

          let angle = th1 === seg1.arc ? seg2.angle : 2;
          let ang1 = angs[i+1];
          let ang2 = angs[i];

          let sgn1 = vrels.get(v1) === REL.INLINE ? +1 : -1;
          let sgn2 = vrels.get(v2) === REL.INLINE ? +1 : -1;
          if ( sgn1 > 0 && sgn2 < 0 ) {
            // cross meet
            spins.set([     -ang1, knife, theta, +1], v1);
            spins.set([     -ang2, efink, theta, -1], v2);

          } else if ( sgn1 < 0 && sgn2 > 0 ) {
            // cross meet
            spins.set([ -(2+ang1), efink, theta, +1], v1);
            spins.set([    2-ang2, knife, theta, -1], v2);

          } else if ( sgn1 > 0 && ang2 + angle > 3 ) {
            // scoop meet
            spins.set([     -ang1, knife, theta, +1], v1);
            spins.set([    2-ang2, knife, theta, -1], v2);

          } else if ( sgn1 < 0 && ang2 + angle > 1 ) {
            // scoop meet
            spins.set([ -(2+ang1), efink, theta, +1], v1);
            spins.set([     -ang2, efink, theta, -1], v2);
      ...
      if ( It.keys(spins).every(a => a[1]!==knife) || It.keys(spins).every(a => a[1]!==efink) ) {
        if ( spins.size > 0 ) {
          // only one side of the knife has no meet
          let [spin0, vertex0] = It.entries(spins).first();
          let theta0 = spin0[2];
          let elem0 = elements.find(elem => elem.has(vertex0[0]));

          if ( spin0[1] === efink ) {
            spins.set([+2, knife, theta0, -1], elem0);
            spins.set([-2, knife, theta0, +1], elem0);
          } else {
            spins.set([+2, efink, theta0, -1], elem0);
            spins.set([-2, efink, theta0, +1], elem0);
          }

        } else if ( vmeets.length === 0 ) {
          // no meet with both sides of the knife
          let point = knife.vectorAt(0);
          let elem0 = this.grab(point, elements);

          console.assert(!Array.isArray(elem0));
          if ( elem0 instanceof Set ) {
            spins.set([+2, knife, 0, -1], elem0);
            spins.set([-2, knife, 0, +1], elem0);
            spins.set([+2, efink, 0, -1], elem0);
            spins.set([-2, efink, 0, +1], elem0);
          }
    ...
    let inner_dashes = new Map(); // spin -> dash
    let outer_dashes = new Map(); // spin -> dash
    for ( let [sgn, circle, dashes] of [[+1, knife, inner_dashes], [-1, efink, outer_dashes]] ) {
      let ameets = It.keys(spins).filter(a => a[1] === circle).toArray()
                     .sort(([,,th1,p1], [,,th2,p2]) => sgn*(th1-th2) || p1-p2);
      console.assert(ameets.length % 2 === 0);

      // make dashes
      for ( let i=0; i<ameets.length; i++ ) if ( ameets[i][3] === +1 ) {
        let a1 = ameets[i];
        let a2 = ameets[i+1] || ameets[0];
        console.assert(a2[3] === -1);

        let arc = sgn > 0 ? a2[2] - a1[2] : a1[2] - a2[2];
        if ( ameets[i+1] === undefined )  arc += 4;
        let angle = -a1[0];
        let radius = circle.radius;
        let orientation = q_spin(circle.orientation, sgn*a1[2]);
        let dash = {arc, angle, radius, orientation};

        if ( Array.isArray(spins.get(a1)) ) {
          console.assert(Array.isArray(spins.get(a2)));
          dash.prev = [-a1[0], ...spins.get(a1)];
          dash.next = [-a2[0], ...spins.get(a2)];
          dash.siblings = elements.find(elem => elem.has(dash.prev[1]));
          console.assert(dash.siblings.has(dash.next[1]));
        } else {
          dash.prev = [-a1[0], dash, dash.arc, -1];
          dash.next = [-a2[0], dash,        0, +1];
          dash.siblings = spins.get(a1);
          console.assert(spins.get(a1) === spins.get(a2));
        }
      ...
      for ( let i=0; i<ameets.length; i++ ) if ( ameets[i][3] === -1 ) {
        ...
        let a2 = ameets[i+1] || ameets[0];
        console.assert(a2[3] === +1);
        let dash1 = dashes.get(a1);
        let dash2 = dashes.get(a2);

        if ( a1[2] === a2[2] ) {
          dash1.forward = dash2;
          dash2.backward = dash1;

        } else {
          let vs1 = vertices.find(vs => vs.includes(dash1.next));
          let vs2 = vertices.find(vs => vs.includes(dash2.prev));
          let rel1 = sgn > 0 ? REL.INLINE : REL.OUTLINE;
          let rel2 = sgn > 0 ? REL.OUTLINE : REL.INLINE;
          let v1 = vs1.find(v => v[2] === +1 && crels.get(v[0]) === rel1);
          let v2 = vs2.find(v => v[2] === -1 && crels.get(v[0]) === rel2);
          if ( v1 !== undefined ) dash1.forward = v1[0];
          if ( v2 !== undefined ) dash2.backward = v2[0];
        }
      }
    }
    ...
    // make zippers
    let zippers = []; // [[dash1, dash2, offset], ...]
    {
      thetas = thetas.sort();
      for ( let i=0; i<thetas.length; i++ ) {
        let j = (i+1)%thetas.length;
        let th1 = thetas[i];
        let th2 = thetas[j];
        let offset = i+1 < list.length ? 0 : 4;
        zippers_.push([[,,th1,], [,,th2,], offset]);
      }

      // zippers_ = [[[,,th1,], [,,th2,], dth], ...]  :  overlay region = th2 - th1 + dth

      let i1 = 0, a1;
      for ( let a of inner_dashes.keys() ) {
        if ( a[3] === +1 ) {

          while ( zippers_[i1][0][2] !== a[2] )
            i1++;
          a1 = a;
          ...
          do {
            z[0] = a1;
            z[2] += (i1 < zippers_.length ? 0 : 4);
            i1++;
            z = zippers_[i1%zippers_.length];
          } while ( z[0][2] !== a[2] );

        }
      }

      let i2 = zippers_.length-1, a2;
      for ( let a of outer_dashes.keys() ) {
        if ( a[3] === +1 ) {

          while ( zippers_[i2][1][2] !== a[2] )
            i2--;
          a2 = a;
          ...
          do {
            ...
            z[2] += (i2 >= 0 ? 0 : 4);
            i2--;
            z = zippers_[(i2+zippers_.length)%zippers_.length];
          } while ( z[1][2] !== a[2] );

        }
      }

      // zippers_ = [[a1, a2, dth], ...]  :  offset = a2[2] - a1[2] + dth

      console.assert(zippers_.every(([a1, a2]) => !!a1[1] === !!a2[1]));
      for ( let [a1, a2, dth] of zippers_ ) if ( a1[1] !== undefined && a2[1] !== undefined ) {
        let dash1 = inner_dashes.get(a1);
        let dash2 = outer_dashes.get(a2);
        let offset = a2[2] - a1[2] + dth;
        zippers.push([dash1, dash2, offset]);

        console.assert(offset > 0 && offset < dash1.arc + dash2.arc);
        console.assert((dash1.arc !== offset) === It.keys(angle).some(a => a[2]===a2[2] && a[3]===-1 && outer_dashes.get(a)===a1));
        console.assert((dash2.arc !== offset) === It.keys(angle).some(a => a[2]===a1[2] && a[3]===-1 && inner_dashes.get(a)===a2));
      }
    }
    ...
    let inner = new Set();
    let outer = new Set();
    for ( let vs of vertices ) for ( let v of vs ) {
      let rel = vrels.get(v);
      if ( Object.is(rel, +0) || rel === +1 )
        inner.add(v[0]);
      if ( Object.is(rel, -0) || rel === -1 )
        outer.add(v[0]);
    }
    return [zippers, inner, outer];
  }
  /**
   * Find the element contains given point.
   *
   * @param {number[]} point
   * @param {Array<Set<SphSeg>>} elements
   * @returns {object} Vertex or element or null.
   */
  grab(point, elements) {
    elements = Array.from(elements);
    let boundaries = elements.flatMap(elem => Array.from(elem));
    ...
    for ( let segment of boundaries )
      if ( this.meetWithPoint(segment, point) === 0 ) {
        ...
        let theta = offsets.find(offset => this.cmp(point, segment.vectorAt(offset)) === 0)
                    || this.mod4(segment.thetaOf(point));
        if ( theta < segment.arc )
          return [segment, theta, +1];
      }


    // grab element
    let orientation = q_mul(q_align(point, boundaries[0].vertex), [0.5, 0.5, 0.5, -0.5]);
    let knife = new SphCircle({orientation, radius:1});

    // meet with points
    let vertices = [];
    let vrels = new Map(); // vertex -> +0/-0/+1/-1
    for ( let segment of boundaries ) for ( let offset of [0, ...segment.adj.keys()].slice(0, -1) )
      if ( Array.from(vrels.keys()).every(k => !Array.equal(k, [segment, offset, +1])) ) {
        let vgroup = Array.from(this.spin(segment, offset, +1)).map(a => a.slice(1))
                          .filter(v => boundaries.includes(v[0]));
        if ( offset !== 0 && vgroup.length === 2 )
          continue;

        let rel = this.meetWithPoint(knife, segment.vectorAt(offset));
        for ( let v of vgroup ) {
          console.assert(Array.from(vrels.keys()).every(k => !Array.equal(k, v)));
          console.assert(rel === this.meetWithPoint(knife, v[0].vectorAt(v[1])));
          vrels.set(v, rel);
        }
        vertices.push(vgroup);
    ...
    // meet with circles
    let crels = new Map(); // segment -> +0/-0/+1/-1/null
    for ( let segment of boundaries ) {
      let children = Array.from(vrels.keys()).filter(k => k[0] === segment);
      let [rel, meets] = this.meetWithCircle(knife, segment);
      crels.set(segment, rel);

      if ( rel === 0 ) {
        console.assert(children.every(v => vrels.get(v) === 0));
        for ( let v of children ) vrels.set(v, rel);
        ...
        // snap to known meeted vertices
        let snapped = {[+1]:[], [-1]:[]};
        for ( let v of children ) if ( vrels.get(v) === 0 ) {
          let p = v[0].vectorAt(v[1]);
          let [meet, dis] = meets.map(m => [m, disL1(m, p)])
                                 .sort((a, b) => a[1]-b[1]).shift();
          console.assert(!snapped[v[2]].includes(meet));
          if ( fzy_cmp(dis, 0, this.tol*10) )
            console.warn(`calculation error is too large: ${dis}`);

          snapped[v[2]].push(meet);
          vrels.set(v, 0 * (rel || (meets[0]===meet ? v[2] : -v[2])));
        }

        // add vertices of meet points
        for ( let meet of meets ) if ( !snapped[+1].includes(meet) && !snapped[-1].includes(meet) ) {
          let offset = this.mod4(segment.thetaOf(meet));
          if ( offset >= segment.arc )
            continue;
          ...
          for ( let v of vgroup ) {
            console.assert(v[0] === segment || !crels.has(v[0]));
            console.assert(Array.from(vrels.keys()).every(k => !Array.equal(k, v)));
            console.assert(0 === this.meetWithPoint(knife, v[0].vectorAt(v[1])));

            let rel0 = 0;
            if ( v[0] === segment && v[1] === offset )
              rel0 = 0 * (rel || (meets[0]===meet ? v[2] : -v[2]));
            vrels.set(v, rel0);
          }
          vertices.push(vgroup);
        }
    ...
    // determine meeted angles
    let angles = new Map(); // [ang, knife/efink, th, p] -> vertex/element
    {
      let vmeets = vertices.filter(([v]) => vrels.get(v) === 0);
      for ( let vgroup of vmeets ) {
        let theta = this.mod4(knife.thetaOf(vgroup[0][0].vectorAt(vgroup[0][1])));
        ...
        for ( let v of vgroup ) {
          ...
          let sgn = Object.is(vrels.get(v), +0) ? +1 : -1;
          let dv = rotate([Math.cos((th+p)*Q), Math.sin((th+p)*Q), 0], seg.orientation);
          let ang = angleTo(dv, dv0)/Q;
          if ( crels.get(seg) !== null ) ang = (this.abs4(ang)<1 ? 0 : 2) * sgn;
          else if ( sgn > 0 )            ang = this.mod4(ang+1)-1;
          else                           ang = this.mod4(ang+3)-3;
        ...
        for ( let i=0; i<vgroup.length; i+=2 ) {
          ...
          let sgn1 = Object.is(vrels.get(v1), +0) ? +1 : -1;
          let sgn2 = Object.is(vrels.get(v2), +0) ? +1 : -1;
          ...
          if ( sgn1 > 0 && sgn2 < 0 ) {
            // cross meet
            angles.set([     -ang1, knife, theta, +1], v1);
            angles.set([     -ang2, efink, theta, -1], v2);

          } else if ( sgn1 < 0 && sgn2 > 0 ) {
            // cross meet
            angles.set([ -(2+ang1), efink, theta, +1], v1);
            angles.set([    2-ang2, knife, theta, -1], v2);
      ...
      // special cases
      if ( angles.every(a => a[1]!==knife) || angles.every(a => a[1]!==efink) ) {
        if ( angles.size > 0 ) {
          let [[,c0,th0], [seg0]] = angles.entries().next().value;

          if ( c0 === efink ) {
            angles.set([2, knife, th0, -1], elements.find(elem => elem.has(seg0)));
            angles.set([2, knife, th0, +1], elements.find(elem => elem.has(seg0)));
          } else {
            angles.set([2, efink, th0, -1], elements.find(elem => elem.has(seg0)));
            angles.set([2, efink, th0, +1], elements.find(elem => elem.has(seg0)));
          }

        } else if ( vmeets.length === 0 ) {
          let point = knife.vectorAt(0);
          let target = this.grab(point, elements);

          console.assert(target instanceof Set || target === null);
          if ( target instanceof Set ) {
            angles.set([2, knife, 0, -1], target);
            angles.set([2, knife, 0, +1], target);
            angles.set([2, efink, 0, -1], target);
            angles.set([2, efink, 0, +1], target);
          }
  ...
  // meetWithElementsShortcutted(knife, elements) {}
  // grabShortcutted(knife, elements) {}
  ...
  groupByAdjacency(...elements) {
    ...
    for ( let elem of unprocessed ) {
      ...
      let group = [elem];
      let segs = Array.from(elem);

      for ( let seg of segs ) for ( let adj_seg of seg.adj.values() ) {
        let i = boundaries.findIndex(bd => bd.includes(adj_seg));
        if ( i !== -1 && unprocessed.has(elements[i]) ) {
          unprocessed.delete(elements[i]);
          group.push(elements[i]);
          segs.push(...elements[i]);
        }
      }
  ...
  groupByTwistability(...elements) {}
  // merge(elems) {}
  separateByConnectivity(element) {}
  // split(elems) {}
  ...
  /**
   * Glue zipper.  The glued segments should belong to same host.
   *
  ...
  drawZipper(elements, circle) {
    ...
    let zipper = this.meetWithRegions(circle, elements);
    let inner = [], outer = [];
  ...
  cutAlongZipper(dashes) {
    ...
    if ( dashes.length == 1 && dashes[0][0].element ) {
      ...
      let circle_ = new SphCircle(circle).flip();
  ...
  sketchProfile(segments) {
    ...
    for ( let contact of uncovered ) {
      ...
      let {radius, orientation} = seg.circle.shift(th2).flip();
  ...
  */
   ...
   * @returns {boolean} True if this point is locally twistable.
   */
  isLocallyTwistable(segment, offset, boundaries=seg.siblings) {
   ...
   */
  isTwistable(segment, boundaries=segment.siblings, global_test=false) {
    boundaries = Array.from(boundaries);
  ...
  partitionByTwistability(elements, global_test=false) {
    ...
    do {
      for ( i=0; i<segments.length; i++ ) if ( !untwistable.has(segments[i]) ) {
        let seg0 = segments[i];
        let fixed = Array.from(seg0.siblings);
    ...
    for ( let elem0 of unprocessed ) {
      ...
      for ( let elem of comb )
        for ( let seg of elem.boundaries )
          if ( untwistable.has(seg) )
            for ( let adj_seg of seg.adj.keys() )
              if ( unprocessed.delete(adj_seg.siblings) )
                comb.push(adj_seg.siblings);
      res.push(comb);
    }
  ...
  partitionByBoundaries(...fences) {
    ...
    for ( let bd of unprocessed ) {
      ...
      for ( let par_seg of bd )
        region.elements.add(par_seg.siblings);
      ...
      for ( let elem of region.elements ) for ( let seg of elem.boundaries ) {
        ...
        if ( bd === undefined ) {
          for ( let adj_seg of seg.adj.keys() )
            region.elements.add(adj_seg.siblings);
